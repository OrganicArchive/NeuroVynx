 ...
 * ==============================================
 * Renders spatial distribution of EEG band power using a high-fidelity 
 * heatmap interpolation model.
 * 
 * Mathematical Approach:
 * - Interpolation: Utilizes Shepard's Inverse Distance Weighting (p=2.0) to 
 *   project discrete sensor measurements onto a continuous 64x64 grid.
 * - Visualization Modes:
 *    1. Relative Power (REL): Sequential scale (Indigo-Emerald-Rose) highlighting regional dominance.
 *    2. Normative Deviation (DEV): Diverging Z-score map (Blue-White-Red) centered at 0.0 (Reference).
 * - Spatial Masking: Enforces a spherical boundary with anatomical orientors (Nasion, Left, Right).
 */
...
const BrainTopomap: React.FC<BrainTopomapProps> = ({ 
  ...
  useEffect(() => {
    if (!canvasRef.current || !gridData || gridData.length === 0) return;
    ...
    if (viewMode !== 'electrodes' && gridData && gridData.length > 0) {
      ...
      if (bCtx) {
        ...
        for (let y = 0; y < gridSize; y++) {
          // V-FLIP FIX: Backend gridData[0] is posterior (-1.1), but canvas y=0 is top (anterior).
          // We reverse the row access to align the heatmap with anatomical coordinates.
          const row = gridData[(gridSize - 1) - y];
          if (!row) continue;
          for (let x = 0; x < gridSize; x++) {
            const rawVal = row[x] ?? 0;
            let normVal: number;
            
             if (mapType === 'normative_z_map') {
              // Center at 0.5 (White), range is [-limit, +limit]
              normVal = 0.5 + (rawVal / (2 * (symmetricLimit || 2.0)));
            } else {
    ...
    // 2. Draw Scalp Outlines & Orientation
    drawScalpOutline(ctx, centerX, centerY, radius, trustLevel);

    // 3. Draw Electrodes (Small focal dots)
    if (electrodes && electrodes.length > 0) {
      drawElectrodes(ctx, electrodes, centerX, centerY, radius, trustLevel, viewMode !== 'heatmap');
    }

  }, [gridData, electrodes, trustLevel, vMin, vMax, viewMode, mapType, symmetricLimit]);

  const getColorForValue = (val: number) => {
    const v = isFinite(val) ? Math.max(0, Math.min(1, val)) : 0.5;
    // High-Contrast Research Palette: Deep Blue -> Emerald -> Amber -> Crimson
    if (v < 0.25) return lerpColor({ r: 30, g: 40, b: 120 }, { r: 50, g: 180, b: 255 }, v / 0.25);
    if (v < 0.5) return lerpColor({ r: 50, g: 180, b: 255 }, { r: 16, g: 185, b: 129 }, (v - 0.25) / 0.25);
    if (v < 0.75) return lerpColor({ r: 16, g: 185, b: 129 }, { r: 245, g: 158, b: 11 }, (v - 0.5) / 0.25);
    return lerpColor({ r: 245, g: 158, b: 11 }, { r: 220, g: 38, b: 38 }, (v - 0.75) / 0.25);
  };

  const getNormativeColor = (val: number) => {
    const v = isFinite(val) ? Math.max(0, Math.min(1, val)) : 0.5;
    // Normative Alpha: Blue (Reduced) -> White (Reference) -> Red (Elevated)
    const deadband = 0.05;
    if (v >= 0.5 - deadband && v <= 0.5 + deadband) return { r: 255, g: 255, b: 255 };

    if (v < 0.5) {
       const f = v / 0.5;
       return lerpColor({ r: 10, g: 40, b: 180 }, { r: 255, g: 255, b: 255 }, f);
    }
    const f = (v - 0.5) / 0.5;
    return lerpColor({ r: 255, g: 255, b: 255 }, { r: 180, g: 10, b: 10 }, f);
  };
  ...
  const drawElectrodes = (ctx: CanvasRenderingContext2D, electrodes: any[], cx: number, cy: number, r: number, trust: string, showColors: boolean) => {
    electrodes.forEach(e => {
      ...
      const rawVal = (e.value && metricLabel) ? (e.value[metricLabel.toLowerCase()] ?? 0) : 0;
      let normVal: number;
      if (mapType === 'normative_z_map') {
        normVal = 0.5 + (rawVal / (2 * (symmetricLimit || 2.0)));
      } else {
