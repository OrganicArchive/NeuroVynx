...
from app.eeg.qeeg.interpretation.recording_engine import run_recording_interpretation
...
def get_session_segment(
    ...
    file_path = ensure_valid_path(session.file_path)
...
@router.get("/{session_id}/analysis/recording")
def get_recording_analysis(
    session_id: str,
    db: DBSession = Depends(get_db)
):
    """
    Returns a recording-level interpretive summary by sampling representative windows.
    """
    try:
        return run_recording_interpretation(db=db, session_id=session_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Recording analysis error: {str(e)}")
